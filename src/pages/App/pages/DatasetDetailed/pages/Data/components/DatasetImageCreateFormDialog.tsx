import { useCreateDatasetImages} from "../../../../../../../api/app/datasets.ts";
...
function DatasetCreateFormDialog({dialog}: Props) {
    ...
    const {mutateAsync} = useCreateDatasetImages(parseInt(dataset_id || ""))({
        onSuccess: () => (
    ...
    return (
        <Dialog size="lg" dialog={dialog} title="Create new Dataset">
            <FormikForm className="grid gap-2" initialValues={{image: ""}} onSubmit={async (values, helpers) => {
                await mutateAsync(values)
                dialog.setOpen(false)
                helpers.resetForm()
            }}>
                <FormikFileInput type="file" placeholder="Name of the dataset" name="image" label="Name"></FormikFileInput>
